...
(function () {
    ...
    // Optimized web-delivery variants (generated by tools/optimize-images.mjs).
    // Keyed by the original artwork path so the data file stays untouched.
    var OPT = (window.RAQEEM_OPT && window.RAQEEM_OPT.images) || {};
    var THUMB_SIZES = '(max-width: 699px) 76vw, (max-width: 1099px) 40vw, 26vw';
    var LEAF_SIZES = '(max-width: 699px) 58vw, (max-width: 1099px) 30vw, 20vw';

    function optOf(src) { return src ? (OPT[src] || null) : null; }

    // Point a thumbnail <img> at the light web variants without touching layout.
    function applyThumb(imgEl, obj, sizes) {
        var o = optOf(obj.src);
        if (o) {
            imgEl.src = o.t400 || o.full || obj.src;
            if (o.t800) {
                imgEl.srcset = o.t400 + ' 400w, ' + o.t800 + ' 800w';
                imgEl.sizes = sizes || THUMB_SIZES;
            }
        } else {
            imgEl.src = obj.src;
        }
    ...
    function makeImg(src, alt, w, h, eager, kind) {
        ...
        if (o) {
            if (kind === 'thumb') {
                i.src = o.t400 || o.full || src;
                if (o.t800) {
                    i.srcset = o.t400 + ' 400w, ' + o.t800 + ' 800w';
                    i.sizes = THUMB_SIZES;
                }
            } else {
                i.src = o.full || o.t800 || src;
            }
        } else if (src) {
            i.src = src;
        }
    ...
    function makeStack(opts) {
        ...
        main.appendChild(makeImg(opts.main.src, opts.main.alt, opts.main.w, opts.main.h, opts.eager, 'thumb'));
        ...
        function applyLeaf(imgEl, obj) {
            if (!obj) return;
            applyThumb(imgEl, obj, LEAF_SIZES);
    ...
    function populateProcessArt() {
        ...
        arts.forEach(function (fig, i) {
            ...
            if (!work) return;
            fig.appendChild(makeImg(work.src, 'عمل من أعمال رقيم', work.w, work.h, false, 'thumb'));
        });
    }
    ...
    function buildGallery(cat) {
        ...
        items.forEach(function (w, i) {
            ...
            var im = makeImg(w.src, alt, w.w, w.h, i < 3, 'full');
    ...
    function buildCarouselList(cat) {
        ...
        // Lenis hijacks wheel/touch globally (and preventDefaults while stopped).
        // Exclude this native scroll container so genuine scrolling reaches it.
    ...
    function buildCarouselView(cat, idx) {
        ...
        // Only the current slide and its immediate neighbours are fetched.
        // An entire large carousel is never downloaded in one go.
        proj.slides.forEach(function (s, i) {
            var slide = make('figure', 'carousel__slide');
            var im = makeImg('', proj.title + ' — ' + (i + 1), s.w, s.h, false, 'full');
            slide.appendChild(im);
            track.appendChild(slide);
            slides.push(slide);
            imgs.push(im);
        ...
        function loadSlide(i) {
            if (i < 0 || i >= imgs.length || loaded[i]) return;
            loaded[i] = true;
            var o = optOf(proj.slides[i].src);
            imgs[i].loading = 'eager';
            imgs[i].src = (o && (o.full || o.t800)) || proj.slides[i].src;
        }
        ...
        function align(i) {
            ...
            // Fetch the current slide plus its immediate neighbours only.
            loadSlide(index - 1);
            loadSlide(index);
            loadSlide(index + 1);
        }
